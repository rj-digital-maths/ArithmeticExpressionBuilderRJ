...
def generate_quiz(count, level):
    ...
    for _ in range(count):
        ...
        quiz.append({"numbers": numbers, "target": target, "pattern": pattern})
    return quiz

# =========================================================
# 📄 PDF GENERATOR
# =========================================================
def generate_pdf(name, student_class, difficulty, score, total, quiz_data, user_answers):
    buffer = BytesIO()
    ...
    for i, q in enumerate(quiz_data):
        ans = user_answers.get(f"ans_{i}", "").strip()
        is_correct = False
        try:
...
if not st.session_state.quiz_generated:
    ...
    # 🔑 LICENSE KEY BOX
    license_key = st.text_input("License Key (Leave blank for Free Trial Mode)", type="password", help="Enter a premium key to choose your number of questions and unlock reports.")
    
    # Check key against backend registry
    valid_keys = st.secrets.get("premium_keys", ["RJ-MASTER-KEY"])
    is_key_valid = license_key.strip() in valid_keys

    # Handle instantaneous state reloads on password changes
    ...
    # Dynamic Form Controls based on Premium Authorization State
    if st.session_state.is_premium:
        st.success("🔓 Premium Key Verified! Select your custom quiz length below.")
        
        # FIX: Directly wire slider changes into the persistent Session State memory space
        q_count = st.select_slider("Number of Questions", options=[5, 10, 15, 20, 25], value=10)
        st.session_state.total_qs = q_count
    else:
        st.info("📝 No premium key entered. App will run in Free Trial Mode (Fixed to 3 Questions).")
...
# --- ACTIVE QUIZ SCREEN ---
    ...
    for i, q in enumerate(st.session_state.quiz_data):
        ...
        with col1: st.metric(label="Target Number", value=q['target'])
        with col2:
            cols = st.columns(3)
            shuffled_nums = sorted(q['numbers'], key=lambda k: random.random())
            for idx, num in enumerate(shuffled_nums):
        ...
        answers[f"ans_{i}"] = st.text_input("Your Expression String", key=f"input_{i}", placeholder="enter your expression here.", label_visibility="collapsed")
        st.write("---")
...
# --- SCOREBOARD & REPORT SCREEN ---
    ...
    correct_answers = 0
    for i, q in enumerate(st.session_state.quiz_data):
        ans = st.session_state.user_answers.get(f"ans_{i}", "").strip().replace('−', '-')
        try:
            parsed = [int(n) for n in re.findall(r'\d+', ans)]
            if sorted(parsed) == sorted(q['numbers']) and not re.search(r'[^0-9+\-()\s]', ans):
                if eval(ans, {"__builtins__": None}, {}) == q['target']: correct_answers += 1
        except: pass
    ...
    with col_dl:
        if st.session_state.is_premium:
            pdf_data = generate_pdf(st.session_state.student_name, st.session_state.student_class, st.session_state.difficulty, correct_answers, st.session_state.total_qs, st.session_state.quiz_data, st.session_state.user_answers)
            ...
            if st.button("🔒 Save Result as PDF (Premium)", use_container_width=True):
                st.error("📥 Printable classroom report generation is locked. Please upgrade to unlock premium full features!")
